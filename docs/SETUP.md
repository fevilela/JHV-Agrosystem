...
| `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` | Só p/ upload de fotos/documentos | Credenciais do Supabase Storage (mesmo projeto do `DATABASE_URL`) — usadas por `src/lib/upload.ts` pra enviar/excluir fotos e documentos de animais no bucket `uploads`. A service role key ignora RLS e só é usada server-side. Exige o bucket `uploads` (público) já criado no painel — ver [DEPLOY.md](./DEPLOY.md#uploads) |
