...
import { healthRecordTypeLabels, formatDate } from "@/lib/labels";
import { DeleteButton } from "@/components/crud/delete-button";
import { deleteHealthRecordAction } from "./actions";

export default async function SanidadeListPage() {
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                ...
                <td className="px-4 py-3 text-neutral-700">{healthRecordTypeLabels[r.type]}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link
                      href={`/pecuaria/sanidade/${r.id}`}
                      className="rounded-md p-1.5 text-neutral-400 transition hover:bg-neutral-100 hover:text-neutral-700"
                      title="Editar"
                    >
                      <Pencil size={16} />
                    </Link>
                    <DeleteButton onDelete={deleteHealthRecordAction.bind(null, r.id)} />
                  </div>
                </td>
              </tr>
            ))}
