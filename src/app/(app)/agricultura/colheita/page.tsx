  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Colheita</h1>
          <p className="mt-1 text-sm text-neutral-500">
            {harvests.length} registros · {totalKg.toLocaleString("pt-BR")} kg no total
          </p>
        </div>
        <Link
          ...
          <tbody>
            ...
            {harvests.map((h) => (
              <tr key={h.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                <td className="px-4 py-3 text-neutral-700">{formatDate(h.date)}</td>
                ...
                <td className="px-4 py-3 text-neutral-700">
                  {h.producaoKg ? `${h.producaoKg} kg` : "—"}
                </td>
                <td className="px-4 py-3 text-neutral-700">{h.umidade ? `${h.umidade}%` : "—"}</td>
