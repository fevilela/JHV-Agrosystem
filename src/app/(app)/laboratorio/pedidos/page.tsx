...
export default async function PedidosAnaliseListPage() {
  const { organizationId, organization } = await requireModule("laboratorio");
  const locale = await getLocale();
  const pedidos = await prisma.pedidoAnalise.findMany({
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Pedidos e Faturamento</h1>
          <p className="mt-1 text-sm text-neutral-500">
            {pedidos.length} {pedidos.length === 1 ? "pedido cadastrado" : "pedidos cadastrados"}
          </p>
        </div>
        <Link
          href="/laboratorio/pedidos/novo"
            ...
            {pedidos.map((p) => (
              <tr key={p.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                <td className="px-4 py-3">
                  <Link href={`/laboratorio/pedidos/${p.id}`} className="font-medium text-brand-800 hover:underline">
                    {p.numero}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusColor[p.statusFinanceiro]}`}>
                    {pedidoAnaliseStatusFinanceiroLabels[p.statusFinanceiro]}
                  </span>
                </td>
              </tr>
            ))}
